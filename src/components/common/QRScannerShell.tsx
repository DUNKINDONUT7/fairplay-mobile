...
// and scan-frame overlay — only what happens with the scanned value differs,
// which stays in each caller's onBarcodeScanned/footer.
  ...
  if (!permission.granted) {
    return (
      <View style={[styles.shell, styles.centered, { backgroundColor: colors.bg, paddingTop: insets.top + 24 }]}>
        ...
        <Text style={[styles.permissionText, { color: colors.textPrimary }]}>Camera access is needed to scan this QR code.</Text>
        <Pressable
  ...
  return (
    <View style={styles.shell}>
      ...
      <View style={[styles.topOverlay, { paddingTop: insets.top + 12 }]}>
        ...
        <Text style={styles.topOverlayText}>{title}</Text>
        <View style={{ width: 38 }} />
      </View>
      ...
      <View style={[styles.bottomOverlay, { paddingBottom: insets.bottom + 20 }]}>{footer}</View>
    </View>
  );
}
...
export const qrScannerFooterStyles = styles;
